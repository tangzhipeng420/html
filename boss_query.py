#!/usr/bin/env python3
"""BOSS批量查号 - 通过CDP控制SafeBrowser (IPv6 fix)"""
...
def find_page():
    # Try IPv6 first (Windows CDP often on [::1])
    for a in ["[::1]", "127.0.0.1"]:
        try:
            import urllib.request
            pgs = json.loads(urllib.request.urlopen(f"http://{a}:{CDP_PORT}/json",timeout=3).read())
            for p in pgs:
                t = (p.get("title","") or "").lower()
                if any(x in t for x in ["crm","boss","客户关系"]):
                    log(f"Found: {p.get('title','')[:40]}")
                    return p["id"], a
        except: continue
    return None, None
...
def main():
    ...
    if not os.path.exists(SRC):
        log(f"找不到 {SRC}，请把号码放桌面水池.xlsx"); return
    wb = openpyxl.load_workbook(SRC)
    ws = wb.active
    phones = [str(ws.cell(r,1).value or "").strip() for r in range(2, ws.max_row+1)
              if ws.cell(r,1).value is not None and str(ws.cell(r,1).value).strip()]
    if not phones: log("无号码"); return
    log(f"共{len(phones)}个")

    pid, addr = find_page()
    if not pid: log("SafeBrowser未打开或未开启调试模式"); return
    log(f"CDP: {addr}:{CDP_PORT}")
    ...
    for fn in json.loads(aids or "[]"):
        if js(c, m, f"(function(){{var f=document.getElementById('{fn}');try{{var d=f.contentDocument||f.contentWindow.document;return d.getElementById('ACCESS_NUMBER')?'ok':'';}}catch(e){{return'';}}}})()"):
            fid = fn; break
        m+=1
    if not fid: log("找不到查询框 (iframe可能跨域)"); c.close(); return
    log(f"查询框: {fid}")
