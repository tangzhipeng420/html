...
# Helper: find iframe with ACCESS_NUMBER (and optionally keyword in src)
def find_bill_ifr():
    return js('(function(){try{var es=document.querySelectorAll("iframe");for(var i=0;i<es.length;i++){try{var d=es[i].contentDocument;if(d&&d.getElementById("ACCESS_NUMBER")&&!d.getElementById("LOGIN_USER_ID")&&es[i].src.indexOf("payment")>=0){return es[i].id;}}catch(e){}}return"nf";}catch(e){return"err"}})()')

def find_any_bill():
    return js('(function(){try{var es=document.querySelectorAll("iframe");for(var i=0;i<es.length;i++){try{var d=es[i].contentDocument;if(d&&d.getElementById("ACCESS_NUMBER")&&!d.getElementById("LOGIN_USER_ID")){return es[i].id;}}catch(e){}}return"nf";}catch(e){return"err"}})()')

def find_eq_ifr():
    return js('(function(){try{var es=document.querySelectorAll("iframe");for(var i=0;i<es.length;i++){try{var d=es[i].contentDocument;if(d&&d.getElementById("LOGIN_USER_ID")&&d.getElementById("ACCESS_NUMBER")){return es[i].id;}}catch(e){}}return"nf";}catch(e){return"err"}})()')

# Step1: Find or load billing iframe
bid=find_bill_ifr()
lg(f'Search bill ifr: {bid}')
if bid=='nf':
    lg('Nav to billing...')
    ...
    time.sleep(8)
    bid=find_bill_ifr()
    lg(f'After nav bill ifr: {bid}')
    if bid=='nf':
        bid=find_any_bill()
        lg(f'Fallback bill ifr: {bid}')
