...
# Step1: Find billing iframe (with ACCESS_NUMBER, no LOGIN_USER_ID, src contains payment)
bid=js('(function(){try{var es=document.querySelectorAll("iframe");for(var i=0;i<es.length;i++){try{var d=es[i].contentDocument;if(d&&d.getElementById("ACCESS_NUMBER")&&!d.getElementById("LOGIN_USER_ID")&&es[i].src.indexOf("payment")>=0){return es[i].id;}}catch(e){}}return"nf";}catch(e){return"err"}})()')
if bid=='nf':
    # Fallback: any iframe with ACCESS_NUMBER (might be equity)
...
if bid=='nf':
    lg('ERROR: No billing iframe. Click "代收话费" first!')
    c.close();exit()
...
# Find equity iframe (has LOGIN_USER_ID)
